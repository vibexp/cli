...
func newLogin(resolve StoreResolver, getenv config.Getenv) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Authenticate the active context.\n\n" +
			"By default this opens a browser for interactive OAuth 2.1 login and\n" +
			"stores rotating tokens. With --with-api-key the key is read from a\n" +
			"hidden prompt (TTY) or stdin — never a command-line argument, which\n" +
			"would leak into shell history. Credentials are stored per context in\n" +
			"~/.vibexp/credentials.json (0600).",
		Args: cobra.NoArgs,
...
// runAPIKeyLogin implements `auth login --with-api-key`.
func runAPIKeyLogin(cmd *cobra.Command, resolve StoreResolver) error {
	ctx := cmd.Context()
