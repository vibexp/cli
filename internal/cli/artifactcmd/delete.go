...
func newDelete(resolve resource.CredResolver, getenv config.Getenv) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long:  "Delete an artifact by slug within the resolved project (--project, VIBEXP_PROJECT, or the active context).",
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			slug := args[0]
			proceed, err := resource.ConfirmDeletion(cmd, "artifact "+slug, yes)
			if err != nil {
				return err
			}
			if !proceed {
			...
			cmd.PrintErrf("Deleted artifact %s.\n", slug)
			return nil
		},
	}
